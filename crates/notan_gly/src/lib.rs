...
impl<F: Font + Sync, H: BuildHasher> GlyphBrush<F, H> {
    ...
    pub fn draw_queued_with_transform(
        ...
        self.pipeline.draw(gfx, transform, None);
    ...
    pub fn draw_queued_with_transform_and_scissoring(
        ...
        self.process_queued(gfx);
        self.pipeline.draw(gfx, transform, Some(rect));

        Ok(())
    }

    fn process_queued(&mut self, gfx: &mut Graphics) {
        ...
        loop {
            brush_action = self.glyph_brush.process_queued(
                |rect, tex_data| {
                    ...
                    pipeline.update_cache(gfx, offset, size, tex_data);
                },
            ...
            match brush_action {
                ...
                Err(BrushError::TextureTooSmall { suggested }) => {
                    ...
                    pipeline.increase_cache_size(gfx, new_width, new_height);
        ...
        match brush_action.unwrap() {
            BrushAction::Draw(verts) => {
                self.pipeline.upload(gfx, &verts);
            }
            BrushAction::ReDraw => {}
        };
    }
...
impl<F: Font, H: BuildHasher> GlyphBrush<F, H> {
    fn new(gfx: &mut Graphics, raw_builder: glyph_brush::GlyphBrushBuilder<F, H>) -> Self {
        ...
        GlyphBrush {
            pipeline: GlyPipeline::new(gfx, cache_width, cache_height).unwrap(),
            glyph_brush,
        }
    }
}
