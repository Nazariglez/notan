...
use winit::event::{ElementState, KeyEvent as WKeyEvent};
use winit::keyboard::Key;

pub(crate) fn process(window_id: WindowId, event: WKeyEvent) -> KeyboardEvent {
    let key = key_id(event.logical_key);
    match event.state {
...
fn key_id(keycode: winit::keyboard::Key) -> KeyCode {
    // match keycode {
    //     Key::Named(_) => {}
    //     Key::Character(_) => {}
    //     Key::Unidentified(_) => KeyCode::Unknown,
    //     Key::Dead(_) => KeyCode::Unkown,
    // }
