...
use crate::{cargo_build, copy_assets, DynError, gz_file, project_root, wasm_bindgen, wasm_opt};
...
impl Example {
    pub(crate) fn run_web(self) -> Result<(), DynError> {
        ...
        let input = dist_web_dir(self.release).join(wasm.as_str()).to_string_lossy().into_owned();
        let output = docs_web_dir(self.release, name_str).to_string_lossy().into_owned();
        ...
        if self.release {
            ...
            let inout = docs_web_dir(self.release, name_str).join(wasm_file.as_str()).to_string_lossy().into_owned();
            ...
            if self.gzip {
                ...
                let wasm_gz_output = docs_web_dir(self.release, name_str).join(wasm_gz).to_string_lossy().into_owned();

                let js_gz_input = docs_web_dir(self.release, name_str).join(format!("{name_str}.js")).to_string_lossy().into_owned();
                let js_gz_output = docs_web_dir(self.release, name_str).join(format!("{name_str}.js.gz")).to_string_lossy().into_owned();
        ...
        let mut file_out = File::create(docs_web_dir(self.release, "").join(example_file.as_str()))?;
